...
class Rational
{
    ...
    private static function selectBestConvergent(array $continuedFraction, \GMP $originalN, \GMP $originalD, int $limit): ?array
    {
        ...
        //At each iteration consider a longer and longer prefix of the list
        for ($i = 1; $i <= count($continuedFraction); ++$i) {
            $continuedFractionPrefix = array_slice($continuedFraction, 0, $i);
            $lastIndex = count($continuedFractionPrefix) - 1;
            $lastDigit = $continuedFractionPrefix[$lastIndex];

            //Is the last fraction in this sequence acceptable? If so, ignore all previous fractions, save this as the
            //best result so far and move on to the next level of the continued fraction
            ...
            //Since we already know that the last value is too large (we tested it with the previous if() statement),
            //we can use a binary search to efficiently reduce the range and then do a linear search for the last few
            //steps.

            //The end element is easy. Now we have to determine the start element.
            ...
            if (null !== $result && gmp_div_r($lastDigit, 2) == 0 && gmp_cmp($lastDigit, 2) >= 0) {
                $continuedFractionPrefix[$lastIndex] = $first;
                $fraction = self::getFraction($continuedFractionPrefix);
            ...
            //Test that the first fraction is acceptable. If it is not, then it means that the value in $result is the
            //last acceptable one and we are done
            $continuedFractionPrefix[$lastIndex] = $first;
            $fraction = self::getFraction($continuedFractionPrefix);
            if (gmp_cmp($fraction['den'], $limit) > 0) {
                break;
            ...
            for ($j = $first; $j <= $last; $j = gmp_add($j, 1)) {
                $continuedFractionPrefix[$lastIndex] = $j;
                $fraction = self::getFraction($continuedFractionPrefix);
                if (gmp_cmp($fraction['den'], $limit) > 0) {
